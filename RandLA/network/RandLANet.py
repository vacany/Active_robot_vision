...
class Network(nn.Module):
    ...
    def forward(self, end_points):
        ...
        for j in range(self.config.num_layers):
            ...
            features = f_decoder_i  # Features 32 ch
            f_decoder_list.append(f_decoder_i)
        # ###########################Decoder############################

        features = self.fc1(features) # Features 64 ch
        end_points['decoder_features'] = features.clone()

        features = self.fc2(features) # Features 32 ch


        features = self.dropout(features)
        features = self.fc3(features)
        # Features 19 ch ---> classes
...
class Building_block(nn.Module):
    ...
    def forward(self, xyz, feature, neigh_idx):  # feature: Batch*channel*npoints*1
        ...
        # modification made !!!! -- charleschen
        # f_xyz2 = self.mlp2(f_xyz1)
        f_xyz2 = self.mlp2(f_xyz)
        # batch*npoint*nsamples*channel
