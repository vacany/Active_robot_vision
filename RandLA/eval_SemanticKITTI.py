...
parser.add_argument('--test_id', default='04', type=str, help='Predicted sequence id [default: 08]')
parser.add_argument('--result_dir', default= '/mnt/personal/vacekpa2/data/semantic_kitti/randla_0.06/', help='Dump dir to save prediction [default: result/]')
    ...
    def rolling_predict(self):
        ...
        with torch.no_grad():
            ...
            while np.min(min_possibility) <= 0.8:
                ...
                print("Current Possibility: ", np.min(min_possibility), flush=True) # It like progress in %
    ...
    def merge_and_store(self):
        ...
        for j in tqdm(range(N)):
            if FLAGS.infer_type == 'all':
                ...
                probs = self.test_probs[j][proj_inds[0], :] # predictions probabilities
                pred = np.argmax(probs, 1).astype(np.uint32)    # labels
...
# TODO get the one-time batch with everything
