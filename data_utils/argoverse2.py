...
def read_annotation_file(av2_seq_path):
    ...
    print('unpacking Argoverse Annotation to unified format')
    for lidar_idx in tqdm(range(len(lidar_paths))):
...
def get_ego_bbox_argo():
    # TODO technical depth when doing calibration?
...
def prepare_sequence(av2_seq_path):
    ...
    # box = get_ego_bbox_argo()
    # poses[:,:3,-1] -= box[:3]
    os.makedirs(av2_seq_path + '/pose/', exist_ok=True)
    for idx in range(len(poses)):
        np.save(av2_seq_path + f'/pose/{idx:06}.npy', poses[idx])
    ...
    if '/train/' in av2_seq_path or '/val/' in av2_seq_path:
        read_annotation_file(av2_seq_path)



class Argoverse2_Sequence(Basic_Dataprocessor):

    def __init__(self, data_dir, dataset_type='sensor/train', sequence_nbr=0):
    ...
    def __init_preprocessing(self):
        max_idx = len(glob.glob(f"/{self.sequence_path}/sensors/lidar/*.feather")) - 1  # to include zero
        last_lidar_path =  f'/{self.sequence_path}/lidar/{max_idx:06d}.npy'

        if not os.path.exists(last_lidar_path):
            # Maybe split to different init functions...
            prepare_sequence(av2_seq_path=self.sequence_path)

        self.pts_files = sorted(glob.glob(self.sequence_path + "'/lidar/*.npy"))
        self.label_files = sorted(glob.glob(self.sequence_path + "/seg_labels/*.npy"))
        self.instance_files = sorted(glob.glob(self.sequence_path + "/id_mask/*.npy"))
        self.poses = np.load(self.sequence_path + "/poses.npy")

    def get_ego_poses(self):
        return {t : self.poses[t] for t in range(len(self.poses))}

if __name__ == '__main__':
    dataset = Argoverse2_Sequence(data_dir='/home/patrik/patrik_data/argoverse2/')
