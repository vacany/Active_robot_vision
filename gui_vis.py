...
# TAKEN FROM SEMANTIC_KITTI
# https://github.com/PRBonn/semantic-kitti-api/blob/master/auxiliary/laserscanvis.py
...
class Sequence_Visualizer:
  ...
  def update_scan(self):
    # first open data
    data = self.dataset.__getitem__(self.offset)
    pts = data['pts']
    seg_label = data['label_mapped']
    instance = data['instance']
    #todo choose what you want
    new_id_mask = np.load(os.path.dirname(self.dataset.pts_files[self.offset]) + f'/../ego_spread/new_id_mask/{self.offset:06d}.npy')
    # if self.semantics:
    ...
    range_data = np.copy(new_id_mask)
